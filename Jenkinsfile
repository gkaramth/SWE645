        ...
        stage("Build Survey Image") {
            steps {
                script {
                    ...
                    def customImage = docker.build("gkaramth/645-hw2-image:${BUILD_TIMESTAMP}")
                }
            }
        }
        stage("Push img to docker hub") {
            steps {
                script {
                    ...
                    sh "docker push gkaramth/645-hw2-image:${BUILD_TIMESTAMP}"
                }
            }
        }
        stage("Deploy to kubernetes") {
			steps {
                withEnv(["KUBECONFIG=${KUBECONFIG_CREDENTIALS}"]) {
                    sh 'kubectl set image deployment/deployment container-hw2=gkaramth/645-hw2-image:${BUILD_TIMESTAMP} -n default'
                }
            }
        }
    }
